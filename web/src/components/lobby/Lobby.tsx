...
const Lobby: React.FC<Props> = () => {
  ...
  const {teams} = useAppSelector(state => state.room)
  ...
  const [shareBtnText, setShareBtnText] = React.useState("Share")
  React.useEffect(() => {
    if (!teams) {
      return
    }
      ...
      <div className="flex justify-center items-center mt-12">
        {!showGame && (
          <div className="flex flex-col items-center">
            {!canStart && <span className="mb-10">You must have at least 2 players per team to start</span>}
            <div className="flex flex-row items-center gap-8">
              ...
              >
                <div className="flex flex-row justify-center items-center gap-4 uppercase">
                  {shareBtnText}
                  <svg className="h-8 w-8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512">
                    <path
